...
// This will run a few standardized tests, preventing test writers from having to 
// manually re-implement all these permutations.
template<typename PAT_T, typename DST_T>
void testPatternSuccess(std::string const& data, PAT_T const& pat, DST_T const& expected_value) {
  ...
  //First into Nil
  {
    ...
    for(auto c: data) {
      ...
      EXPECT_NE(status, abu::Result::FAILURE);
