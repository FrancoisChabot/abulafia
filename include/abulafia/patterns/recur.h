...
class Recur : public Pattern<Recur<CHILD_PAT_T, ATTR_T>> {
 ...
 public:
  ...
  Recur(Recur const& rhs) : pat_(rhs.pat_) {}

  Recur(Recur&& rhs) : pat_(std::move(rhs.pat_)) {}

  Recur& operator=(CHILD_PAT_T const& rhs) {
    *pat_ = std::make_unique<CHILD_PAT_T>(rhs);
    return *this;
  }
...
#define ABU_Recur_define(var, RECUR_TAG, pattern)         \
  struct RECUR_TAG : public decltype(pattern) {           \
    using pattern_t = decltype(pattern);                  \
    RECUR_TAG(pattern_t const& p) : pattern_t(p) {}       \
    RECUR_TAG(pattern_t&& p) : pattern_t(std::move(p)) {} \
  };                                                      \
  var = pattern;
