...
struct val_visitor {
  ...
  static decltype(auto) visit(T val, VISIT_T&& visitor) {
    if (N == val) {
      return visitor(std::integral_constant<T, N>());
    } else {
...
struct val_visitor<T, MAX_V, MAX_V> {
  template <typename VISIT_T>
  static decltype(auto) visit(T, VISIT_T&& visitor) {
    return visitor(std::integral_constant<T, MAX_V>());
  }
};

template <std::size_t MAX_V, typename VISITOR_T>
decltype(auto) visit_val(std::size_t v, VISITOR_T&& visit) {
  return val_visitor<std::size_t, MAX_V - 1, 0>::visit(v,
